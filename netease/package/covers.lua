...
function M.thumbnail(url)
  ...
  if not host or not(host=='music.126.net' or host:match('%.music%.126%.net$'))then return nil end
  local clean=url:gsub('[?#].*$',''):gsub('^http:','https:')
  return clean..'?param=92y92'
end
...
function M.identity(url)
  local tail=url:match('^https://p[34]%.music%.126%.net(/.*)$')
  if tail then return 'https://p3.music.126.net'..tail,'https://p4.music.126.net'..tail end
  return url
end
function M.new(net,provider,player,now,disk)
  ...
  function C.poll()
    ...
    local c=net.create(url,{async=true,timeout=10000,bufsz=2048,max_redirects=0,
      headers={['Accept-Encoding']='identity',['User-Agent']='CubicNetEase/0.2'}})
