...
local function notify_xiaozhi_app_change(app_id, source)
  if type(app_id) ~= "string" or app_id == "" then
    return false
  end
  -- Temporary firmware stand-in: MCP reports app changes over IPC until
  -- firmware publishes foreground-app notifications directly.
  local bridge = rawget(_G, "XIAOZHI_SERVICE")
  if bridge and bridge.on_app_change then
    local ok = pcall(function()
      return bridge:on_app_change(app_id, source or "mcp")
    end)
    if ok then return true end
  end
  local codec = rawget(_G, "json") or rawget(_G, "sjson")
  if ipc and ipc.send and codec and codec.encode then
    local ok, raw = pcall(codec.encode, {
      app_id = app_id,
      source = source or "mcp",
    })
    if ok and type(raw) == "string" then
      local sent = pcall(function()
        return ipc.send("xiaozhi-service", "on_app_change", raw)
      end)
      return sent
    end
  end
...
local function launch_app(arguments, ctx)
  ...
  timer:alarm(250, tmr.ALARM_SINGLE, function(instance)
    ...
    notify_xiaozhi_app_change(app_id, "mcp")
