...
return function()
  ...
  local function build()
    ...
    U.rule=box(12,216,296,1,0x252529);U.progress=box(12,216,1,2,0x31C27C)
  ...
  function U.qr(url,native)
    if U.qrraw==url and U.ready then return true end
    ...
    U.qrsource=nil;U.qrraw=url
    if not url then return true end
    if url:sub(1,8)=='\137PNG\r\n\26\n'or url:sub(1,2)=='\255\216' then
      local png=url:sub(1,8)=='\137PNG\r\n\26\n'
      local src,err=url,nil
      if png then src,err=(native.qr_image or native.png_image)(url)end
      if not src then U.qrraw=nil;return nil,err end
      U.qrsource=src;U.canvas=lv_img_create(root);lv_img_set_src(U.canvas,src)
      local w,h
      if png then w=url:byte(19)*256+url:byte(20);h=url:byte(23)*256+url:byte(24)
      else w,h=dofile(DIR..'covers.lua').jpeg_size(url)end
      if not w or not h or w>512 or h>512 or #url>98304 then lv_obj_del(U.canvas);U.canvas=nil;U.qrsource=nil;return nil,'二维码尺寸不兼容'end
      local zoom=math.min(256,math.floor(154*256/math.max(w,h)))
      lv_img_set_pivot(U.canvas,0,0);lv_img_set_zoom(U.canvas,zoom)
      lv_obj_set_pos(U.canvas,math.floor((320-w*zoom/256)/2),math.floor(38+(154-h*zoom/256)/2));return true
    end
  ...
  function U.render(S,A)
    ...
    text(U.brand,login and '扫码登录' or playing and 'QQ音乐' or about and About.title or library and '音乐库' or A.list_title or '歌曲')
    ...
    text(U.badge,about and About.badge or playing and ((A.clock_text or '--:--')..' · '..View.rate(A.player.stats)) or
      login and (A.login and ({qq='QQ',wx='微信'})[A.login.mode]or 'QQ') or library and (S.tab..' / '..(S.tab_count or 2))or
    ...
    if login then text(U.hint,'短按 Home 跳过');text(U.time,'左右切换 QQ / 微信');return end
    if about then
      text(U.accountLabel,'当前账户')
      text(U.account,A.nickname or '游客');text(U.accountState,A.uid and '已保存'or '游客模式')
      text(U.time,'上下切页');text(U.hint,A.uid and 'Home 回到播放'or 'Home 扫码登录');return
    end
    if playing then
      ...
      for i,o in ipairs(U.lines)do
        ...
        color(o,selected and 0x31C27C or 0xAAAAAA)
