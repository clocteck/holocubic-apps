...
local function map_metrics(entries, metric_defs)
  ...
  for _, metric in ipairs(metric_defs or {}) do
    ...
    -- Alias order is the fallback priority. A value outside the configured
    -- valid range is ignored so a second sensor can take over (for example,
    ...
    for _, alias in ipairs(metric.aliases or {}) do
      for _, entry in ipairs(entries) do
        ...
        if min_valid == nil and
            (metric.kind == "temperature" or metric.kind == "speed" or metric.kind == "clock") then
          min_valid = 1
        end
        if valid and min_valid ~= nil then
          valid = value >= min_valid
        end
        if valid and metric.max_valid ~= nil then
          valid = value <= metric.max_valid
        end
